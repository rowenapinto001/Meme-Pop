...
function safeReadState(callback) {
    if (!hasExtensionContext()) {
        return;
    }
...
function safeUpdateState(mutator, callback) {
    if (!hasExtensionContext()) {
        return;
    }
...
function isBelieveMode() {
    return appState.settings.accessory === "crown";
}
function pickBelieveMessage(previous = "") {
...
function createHydrationSplash() {
...
function createMemePop(message) {
    ...
    characterButton.addEventListener("click", () => {
        ...
        playTone("click");
        resetAutoHide(true);
        sendRuntimeMessage({ type: "MEMEPOP_CHARACTER_CLICKED" }, (response) => {
            if (!response?.awarded) {
                return;
            }
